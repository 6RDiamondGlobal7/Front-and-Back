...
exports.submitApplication = async (req, res) => {
    ...
    try {
        ...
        const normalizedMiddleInitial = String(middleInitial || '').replace(/[^a-zA-Z]/g, '').charAt(0).toUpperCase() || null;
        ...
        const { data: createdApplicant, error: appError } = await supabase.from('applicant').insert([{ 
            ...
            middle_initial: normalizedMiddleInitial, suffix: suffix ? suffix.substring(0, 10) : null,
        ...
        // A2. Save document metadata in dedicated document table
        const createdDocumentId = generateDocumentId();
        ...
        if (!statusError && newStatus) {
            const factPayload = { applicant_no: applicantNo, status_id: newStatus.status_id, document_id: createdDocumentId };
            if (resolvedJobId) {
...
exports.updateApplicantStatus = async (req, res) => {
    try {
        ...
        if (factData && factData.status_id) {
            ...
            const factInsertPayload = { applicant_no: id, status_id: statusInsert.status_id };
            if (documentData?.document_id) {
                factInsertPayload.document_id = documentData.document_id;
            }
            await supabase.from('applicantfacttable').insert([factInsertPayload]);
        }
        invalidateCacheByScopes(['jobs', 'dashboard', 'reports', 'applicants', 'upcomingInterviews']);
