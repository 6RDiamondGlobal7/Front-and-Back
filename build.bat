...
echo [INFO] - Front-end-HR external app Electron installer build
echo [INFO]   * HR management system with hosted API access
echo [INFO]   * Forgot-password and interview scheduling flows
echo [INFO] Local .env files are required but remain ignored by Git.
echo [INFO] Front-end-HR also requires VITE_API_BASE_URL in .env.production.
echo.
