...
const PERSONAL_INFO_DRAFT_PREFIX = 'applicationPersonalInfoDraft';
...
const ApplicationForm = () => {
  ...
  const draftKey = `${PERSONAL_INFO_DRAFT_PREFIX}:${branch || ''}:${roleId || ''}`;
  ...
  const [formData, setFormData] = useState(() => {
    ...
    return {
      ...merged,
      middleInitial: sanitizeMiddleInitial(merged.middleInitial)
    };
  });
  ...
  useEffect(() => {
    const personalInfoOnly = PERSONAL_INFO_FIELDS.reduce((acc, field) => {
      acc[field] = formData[field] ?? '';
      return acc;
    }, {});
    sessionStorage.setItem(draftKey, JSON.stringify(personalInfoOnly));
  }, [formData, draftKey]);
        ...
        {currentStep === 3 && (
          <div className="af-step-content fade-in">
            ...
            <div className="af-grid">
              ...
              <div className="af-group"><label className="af-label">Middle Initial (Optional)</label><input type="text" name="middleInitial" className="af-input" placeholder="e.g., P" value={formData.middleInitial} onChange={handleChange} maxLength={1} pattern="[A-Za-z]" inputMode="text" /></div>
