...
const RoleDetails = () => {
  ...
  const handleBack = () => {
    try {
      sessionStorage.removeItem(`${PERSONAL_INFO_DRAFT_PREFIX}:${branch || ''}:${roleId || ''}`);
    } catch (_) {
      // no-op
    }
  ...
  useEffect(() => {
    ...
    const fetchRoleDescription = async () => {
      try {
        const roleTitle = roleIdToTitle[roleId] || '';
        if (!roleTitle || !branch) {
          setLiveDescription('');
          setLiveSections({ responsibilities: [], qualifications: [], benefits: [] });
          return;
        }
        ...
        setLiveSections({
          ...
          benefits: normalizeList(match?.benefits)
        });
      } catch {
        setLiveDescription('');
        setLiveSections({ responsibilities: [], qualifications: [], benefits: [] });
    ...
    fetchRoleDescription();
  }, [API_BASE_URL, branch, roleId]);
    ...
    <div className="af-page-container">
      ...
      <div className="af-card">
        <div className="rd-header">
          ...
          <p className="af-subtitle">{subtitleDescription}</p>
        </div>
