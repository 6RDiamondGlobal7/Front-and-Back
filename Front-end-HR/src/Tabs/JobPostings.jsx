...
const withDashPrefixPerLine = (value) => String(value || '')
  .split('\n')
  .map((line) => line.trim())
  .filter(Boolean)
...
const normalizeDashedTextareaInput = (value) => String(value || '')
  .split('\n')
  .map((line) => {
...
const JobPostings = () => {
  ...
  const BRANCH_ORDER = ['Manila', 'Cebu', 'Davao'];
  ...
  const allBranches = useMemo(() => {
    ...
    const orderedKnownBranches = BRANCH_ORDER.filter((branch) => values.includes(branch));
    const extraBranches = values
      .filter((branch) => !BRANCH_ORDER.includes(branch))
      .sort((a, b) => a.localeCompare(b));

    return [...orderedKnownBranches, ...extraBranches];
  }, [jobs]);
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter((job) => {
      ...
      const matchesBranch = job.branch === selectedBranch;
  ...
  const handleListTextareaKeyDown = (event) => {
  ...
  const handleListTextareaFocus = (mode, fieldName) => {
    if (mode === 'create') {
      setCreateForm((prev) => ({
        ...prev,
        [fieldName]: String(prev[fieldName] || '').trim() ? prev[fieldName] : '- '
      }));
      return;
    }
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  onChange={(e) => setCreateForm((prev) => ({ ...prev, responsibilitiesText: normalizeDashedTextareaInput(e.target.value) }))}
                  onKeyDown={handleListTextareaKeyDown}
                  onFocus={() => handleListTextareaFocus('create', 'responsibilitiesText')}
              ...
              <div className="form-group">
                ...
                <textarea
                  className="form-textarea form-textarea-compact"
                  value={createForm.qualificationsText}
                  onChange={(e) => setCreateForm((prev) => ({ ...prev, qualificationsText: normalizeDashedTextareaInput(e.target.value) }))}
                  onKeyDown={handleListTextareaKeyDown}
                  onFocus={() => handleListTextareaFocus('create', 'qualificationsText')}
              ...
              <div className="form-group">
                ...
                <textarea
                  className="form-textarea form-textarea-compact"
                  value={createForm.benefitsText}
                  onChange={(e) => setCreateForm((prev) => ({ ...prev, benefitsText: normalizeDashedTextareaInput(e.target.value) }))}
                  onKeyDown={handleListTextareaKeyDown}
                  onFocus={() => handleListTextareaFocus('create', 'benefitsText')}
              ...
              <div className="view-description-box">
                ...
                <div className="view-sections-grid">
                  <div className="view-section-card">
                    ...
                    {toStringArray(selectedJob.responsibilities).length > 0 ? (
                      <ul>
                        {toStringArray(selectedJob.responsibilities).map((item, idx) => <li key={`resp-${idx}`}>{item}</li>)}
                      </ul>
                    ) : (
                      <p className="view-empty">No responsibilities configured yet.</p>
                    )}
                  </div>
                  <div className="view-section-card">
                    <h4>Qualifications</h4>
                    {toStringArray(selectedJob.qualifications).length > 0 ? (
                      <ul>
                        {toStringArray(selectedJob.qualifications).map((item, idx) => <li key={`qual-${idx}`}>{item}</li>)}
                      </ul>
                    ) : (
                      <p className="view-empty">No qualifications configured yet.</p>
                    )}
                  </div>
                  <div className="view-section-card view-section-wide">
                    <h4>Benefits & Perks</h4>
                    {toStringArray(selectedJob.benefits).length > 0 ? (
                      <ul>
                        {toStringArray(selectedJob.benefits).map((item, idx) => <li key={`benefit-${idx}`}>{item}</li>)}
                      </ul>
                    ) : (
                      <p className="view-empty">No benefits configured yet.</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  onChange={(e) => setEditForm((prev) => ({ ...prev, responsibilitiesText: normalizeDashedTextareaInput(e.target.value) }))}
                  onKeyDown={handleListTextareaKeyDown}
                  onFocus={() => handleListTextareaFocus('edit', 'responsibilitiesText')}
                />
              </div>
              <div className="form-group">
                <label>Qualifications</label>
                <textarea
                  className="form-textarea form-textarea-compact"
                  value={editForm.qualificationsText}
                  onChange={(e) => setEditForm((prev) => ({ ...prev, qualificationsText: normalizeDashedTextareaInput(e.target.value) }))}
                  onKeyDown={handleListTextareaKeyDown}
                  onFocus={() => handleListTextareaFocus('edit', 'qualificationsText')}
                />
              </div>
              <div className="form-group">
                <label>Benefits & Perks</label>
                <textarea
                  ...
                  onChange={(e) => setEditForm((prev) => ({ ...prev, benefitsText: normalizeDashedTextareaInput(e.target.value) }))}
                  onKeyDown={handleListTextareaKeyDown}
                  onFocus={() => handleListTextareaFocus('edit', 'benefitsText')}
                />
              </div>
            </div>
            <div className="modal-footer modal-footer-split">
