...
const Applicants = () => {
  ...
  const branches = [
    ...BRANCH_ORDER.filter((branch) => applicants.some((item) => item.branch === branch)),
    ...Array.from(new Set(applicants.map((item) => item.branch).filter(isSelectableFilterValue)))
  ...
  const filteredData = applicants.filter((app) => {
    ...
    const matchesBranch = app.branch === branchFilter;
